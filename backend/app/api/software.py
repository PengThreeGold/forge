...
@software_bp.route('/software/<int:space_id>/versions/<int:version_id>', methods=['GET'])
@jwt_required()
@admin_required
def get_version(space_id, version_id):
    """获取软件版本详情"""
    # 验证版本属于指定的软件空间
    version = SoftwareVersion.query.filter_by(
        id=version_id,
        space_id=space_id
    ).first_or_404("版本不存在或不属于指定软件空间")
...
@software_bp.route('/software/<int:space_id>/versions/<int:version_id>', methods=['PUT'])
@jwt_required()
@admin_required
def update_version(space_id, version_id):
    """更新软件版本"""
    # 验证版本属于指定的软件空间
    version = SoftwareVersion.query.filter_by(
        id=version_id,
...
@software_bp.route('/software/<int:space_id>/versions/<int:version_id>', methods=['DELETE'])
@jwt_required()
@admin_required
def delete_version(space_id, version_id):
    """删除软件版本"""
    # 验证版本属于指定的软件空间
    version = SoftwareVersion.query.filter_by(
        id=version_id,
...
@software_bp.route('/software/<int:space_id>/versions/<int:version_id>/publish', methods=['PUT'])
@jwt_required()
@admin_required
def publish_version(space_id, version_id):
    """发布/下架软件版本"""
    # 验证版本属于指定的软件空间
    version = SoftwareVersion.query.filter_by(
        id=version_id,
...
@software_bp.route('/software/<int:space_id>/versions/<int:version_id>/download', methods=['GET'])
@jwt_required()
@admin_required
def download_version(space_id, version_id):
    """下载软件版本（管理员）"""
    # 验证版本属于指定的软件空间
    version = SoftwareVersion.query.filter_by(
        id=version_id,
...
def get_public_spaces():
    ...
    ).distinct().order_by(desc(SoftwareSpace.created_at)).all()
    ...
    for space in spaces:
        ...
        if latest_version:
            ...
            space_dict['latest_publish_date'] = latest_version.publish_date.isoformat(
            ) if latest_version.publish_date else None
...
@software_bp.route('/public/space/<int:space_id>/versions', methods=['GET'])
def get_versions_info(space_id):
    """获取软件版本列表（公开API）"""
    space = SoftwareSpace.query.get_or_404(space_id)
...
@software_bp.route('/public/space/<int:space_id>/download/<string:version>', methods=['GET'])
def download_version_public(space_id, version):
    """下载软件版本（公开API）"""
    space = SoftwareSpace.query.get_or_404(space_id)
...
def get_latest_version(space_id):
    ...
    # 获取最新已发布版本，先按发布日期排序，如果没有发布日期则按创建日期排序
    from sqlalchemy import desc, nullslast
    latest_version = SoftwareVersion.query.filter_by(
        space_id=space_id,
        is_published=True
    ).order_by(
        nullslast(SoftwareVersion.publish_date.desc())
    ).order_by(
        desc(SoftwareVersion.created_at)
    ).first()
