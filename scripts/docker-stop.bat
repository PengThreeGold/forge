@echo off
REM Forge 软件发布管理平台 - Docker 停止脚本 (Windows版)

echo 正在停止Forge软件发布管理平台 (Docker)...
...
REM 询问是否保留数据卷
set /p preserve_volumes=是否保留数据卷?(y/n): 
...
if %ERRORLEVEL% neq 0 (
    echo 警告: 停止Docker服务时出现错误。
) else (
    echo Docker服务已成功停止。
)

REM 询问是否删除Docker镜像
set /p remove_images=是否删除Docker镜像?(y/n): 
...
REM 询问是否清理未使用的Docker资源
set /p prune_docker=是否清理未使用的Docker资源?(y/n): 
if /i "%prune_docker%"=="y" (
    echo 正在清理未使用的Docker资源...
    docker system prune -f
    if %ERRORLEVEL% neq 0 (
        echo 警告: 清理Docker资源时出现错误。
    ) else (
        echo Docker资源已成功清理。
    )
)
